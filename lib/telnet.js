...
// https://tools.ietf.org/html/rfc2944
...
// IAC SB AUTHENTICATION SEND SRP CLIENT|ONE_WAY|ENCRYPT_USING_TELOPT SRP CLIENT|ONE_WAY IAC SE
...
// IAC SB AUTHENTICATION IS SRP CLIENT|ONE_WAY|ENCRYPT_USING_TELOPT EXP aa aa aa aa aa aa aa aa ... IAC SE
// IAC SB AUTHENTICATION REPLY SRP CLIENT|ONE_WAY|ENCRYPT_USING_TELOPT CHALLENGE bb bb bb bb bb bb bb bb ...  IAC SE
// IAC SB AUTHENTICATION REPLY SRP CLIENT|ONE_WAY|ENCRYPT_USING_TELOPT ACCEPT yy yy yy yy yy yy yy yy ...  IAC SE

Client.prototype._authentication = function(cmd) {
  var data = cmd.data;
  var i = 0;
  if (cmd.commandCode === COMMANDS.WILL) {
    // IAC SB AUTHENTICATION SEND SRP CLIENT|ONE_WAY|ENCRYPT_USING_TELOPT
  ...
  if (cmd.commandCode === COMMANDS.SB && data[3] === DESC.SEND) {
    // IAC SB AUTHENTICATION NAME "tjw" IAC SE IAC SB AUTHENTICATION IS
    ...
    this.writeBuffer([
      ...
      SRP.AUTH_TYPE,
      SRP.CLIENT, // ?
      SRP.AUTH, // ? COMMANDS.AUTHENTICATION ??
      COMMANDS.IAC,
      COMMANDS.SE
    ]);
    // IAC SB AUTHENTICATION SEND SRP CLIENT|ONE_WAY|ENCRYPT_USING_TELOPT
  ...
  if (cmd.data[3] === DESC.SEND) {
  }
  if (cmd.data[3] === DESC.REPLY) {
  }
  return 3;
};
...
Client.prototype.writeText = function(text) {
  return this.output.write(text, 'ascii');
};

Client.prototype.write8 = function(ch) {
  var b = new Buffer(1);
  b.writeUInt8(ch, 0);
  return this.output.write(b);
};

Client.prototype.write16 = function(ch) {
  var b = new Buffer(2);
  b.writeUInt16BE(ch, 0);
  return this.output.write(b);
};

GIFWriter.prototype.writeBuffer = function(data) {
